			...
			<table id="center-table">
				...
				<%-- <c:if test="${partList.size() eq 0}">
					...
					<tr>
						...
						<c:if test="${partMemberChk > 0 and ptm.member_id ne sessionScope.loginId and info.recruit_end eq 1}"><td><input type="button" class="manner-button" value="평가하기" onclick="location.href='/mannerGo?member_id=${ptm.member_id}&board_idx=${info.board_idx}'"/></td></c:if>
					</tr>
				</c:forEach>
				<c:forEach items="${partList}" var="part">
					<tr>
						...
						<c:if test="${partMemberChk > 0 and part.member_id ne sessionScope.loginId and partMemberChk > 0 and part.assessNo eq 0}"><td><input type="button" class="manner-button" value="평가하기" onclick="location.href='/mannerGo?member_id=${part.member_id}&board_idx=${info.board_idx}'"/></td></c:if>
						</c:if>
						<c:if test="${info.recruit_end eq 0}">
						<c:if test="${partMemberChk > 0 and info.member_id eq sessionScope.loginId}"><td><input type="button" id="elim" class="elim-button" value="강퇴" onclick="location='mealBan?board_idx=${info.board_idx}&member_id=${part.member_id}'"/></td></c:if>
						</c:if>
					</tr>
				</c:forEach>
			</table>
				...
				<tr>
					<td colspan="4" style="text-align: center">
						<c:if test="${info.recruit_end eq 0 and info.member_id ne sessionScope.loginId}">
							<input id="openModalPop" type="button" value="참여신청"/>
						</c:if>
					</td>
				</tr>
				<tr>
					<td colspan="4" style="text-align: center">		
						...
						<input class="bottom-button back-button" type="button" value="돌아가기" onclick="location.href='mealList.go'"/>
					</td>
				</tr>
			</table>	
	   </div>
