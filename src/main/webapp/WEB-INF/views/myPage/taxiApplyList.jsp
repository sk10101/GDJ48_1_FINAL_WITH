	...
	<c:choose>
	   ...
	   <div class="content">
	     ...
	     <table id="top-table">
			<tr>
				<th>
					<img style="position: absolute; left: -40px; top: -50px; width: 90px; filter: drop-shadow(5px 5px 5px rgba(100, 100, 111, 0.4));"  src="./resources/images/3Dtaxi.png" alt="3Dtaxi">${info.subject}
				</th>
			</tr>
		</table>
     	...
     	<table id="main-table">
				...
				<tbody class="taxiApplyList">
						<tr>
							...
							<td><input class="common-button yes-button" type="button" value="수락" onclick="location.href='taxiApplyUpdate?apply_idx=${apply.apply_idx}&board_idx=${apply.board_idx}&category_id=${apply.category_id}&member_cnt=${apply.member_cnt}&status=1&member_id=${apply.member_id}'" /></td>
							<td><input class="common-button no-button" type="button" value="거절" onclick="location.href='taxiApplyUpdate?apply_idx=${apply.apply_idx}&board_idx=${apply.board_idx}&member_cnt=${apply.member_cnt}&member_id=${apply.member_id}&category_id=${apply.category_id}&status=0'" /></td>
						</tr>
				</tbody>
				</c:forEach>
			</table>
