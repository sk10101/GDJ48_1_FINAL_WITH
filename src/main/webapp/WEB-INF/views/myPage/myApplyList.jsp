				...
				<c:forEach items="${myApplyList}" var="his">
					<tbody class="myApplyList">
							<tr>
								...
								<c:if test="${his.recruit_end eq 0}"><td><input type="button" value="취소" onclick="location.href='applyCancle?apply_idx=${apply.apply_idx}&board_idx=${apply.board_idx}'" /></td></c:if>
                        <c:if test="${his.recruit_end eq 1}">   <td><input type="button" value="삭제" onclick="location.href='applyDelete?apply_idx=${apply.apply_idx}&board_idx=${apply.board_idx}'" /></td></c:if>
							</tr>
					</tbody>
				</c:forEach>
